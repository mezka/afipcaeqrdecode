...
import os
from afipcaeqrdecode.convert_pdf_to_image_and_detect_and_decode_qrs import convert_pdf_to_image_and_detect_and_decode_qrs
...
class TestAfipCaeExtractAndDecode(TestCase):
    ...
    def test_extract_images_from_pdf_afip_invoice_and_decode(self):
        ...
        for filename, expected_metadata in filenames_and_expected_metadatas:
            decoded_cae = convert_pdf_to_image_and_detect_and_decode_qrs(os.path.join(os.path.dirname(__file__), 'sample_files', filename))
